...
def model_version(model_name: str) -> str:
    ...
    changing runtime config automatically invalidates cached results.
    """
    ...
    try:
        ...
        return f"de:{de}+en:{en}+textfilter-v8+posfilter-v1+iban-v2+email-v1+phone-v1+url-v1+invoice-v2+tracking-v1+regcode-v1+scope-blocks-v1+datetime-v1+amount-v2"
    except importlib.metadata.PackageNotFoundError:
        return "unknown"
