...
# ---------------------------------------------------------------------------
# drop_salutation_blocklist — greeting super-spans now retained (N15a)
# ---------------------------------------------------------------------------

def test_drop_salutation_blocklist_retains_user_name_greeting():
    """Greeting super-spans with real names (e.g. 'Hallo Tobias') are retained — not in static list."""
    entities = [
    ...
    assert "Hallo Tobias" in values      # not in static list — retained
    assert "Hello Tobias" in values      # not in static list — retained
    assert "Tobias Kienzler" in values   # bare full name — always retained
    assert "Hallo Maxine" not in values  # in static list — filtered
